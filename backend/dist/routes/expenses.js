...
router.post('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['ADMIN']), auth_1.requireHostelAccess, async (req, res) => {
    try {
        ...
        server_1.io.to(`hostel-${hostelId}`).emit('expense-created', {
            expense: {
                ...expense,
                amount: expense.amount.toNumber(),
            },
...
router.put('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), auth_1.requireHostelAccess, async (req, res) => {
    try {
        ...
        const updateData = await (0, calculations_1.triggerRealTimeUpdate)(hostelId, 'expense');
        server_1.io.to(`hostel-${hostelId}`).emit('expense-updated', {
            expense: {
                ...expense,
                amount: expense.amount.toNumber(),
            },
...
router.delete('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['admin']), auth_1.requireHostelAccess, async (req, res) => {
    try {
        ...
        await (0, calculations_1.calculateDailyTotals)(hostelId, existingExpense.date);
        const updateData = await (0, calculations_1.triggerRealTimeUpdate)(hostelId, 'expense');
        server_1.io.to(`hostel-${hostelId}`).emit('expense-deleted', {
            expenseId: id,
            ...updateData,
        });
...
router.get('/categories', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const categories = await prisma_1.prisma.expense.findMany({
            where: { hostelId },
            select: {
                category: true,
            },
